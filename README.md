...
Конструктор:  
`constructor(products: IProduct[])` - В конструктор передается массив товаров, присутствующих в каталоге.
...
`getChosenProduct(): IProduct` - возвращает выбранный товар
`getProduct(productId: string): IProduct` - возвращает товар по заданному id
...
`validate(): Map<string, string>` - валидирует заполненные поля и возвращает ошибки в виде мапы, где ключ - имя поля, значение - описание ошибки 
